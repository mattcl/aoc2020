...
pub fn bench(c: &mut Criterion) {
    ...
    group.bench_function(
        BenchmarkId::new("initialize version 1", "normal"),
        |b| b.iter(|| Initializer::initialize(black_box(&lines)).unwrap().memory_sum())
    );
    ...
    group.bench_function(BenchmarkId::new("initialize version 2", "normal"), |b| {
        b.iter(|| Initializer::initialize_v2(black_box(&lines)).unwrap().memory_sum())
    });

    group.bench_function(BenchmarkId::new("initialize version 2", "memoized"), |b| {
        b.iter(|| Initializer::initialize_v2_memoized(black_box(&lines)).unwrap().memory_sum())
    });
